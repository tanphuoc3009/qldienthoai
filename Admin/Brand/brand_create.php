...
// Hàm tạo mã HSX tự động
function layMaHSX($conn) {
    $sql = "SELECT MaHSX FROM HangSanXuat ORDER BY MaHSX DESC LIMIT 1";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
    $result = $stmt->get_result();
    if ($result->num_rows > 0) {
        $lastId = $result->fetch_assoc()['MaHSX'];
        $num = (int)substr($lastId, 3) + 1;
        return 'HSX' . str_pad($num, 3, '0', STR_PAD_LEFT);
    }
    return 'HSX001';
}
...
if (isset($_POST['create'])) {
    $tenHSX = $_POST['TenHSX'];
    $maHSX = layMaHSX($conn);
    $sql = "INSERT INTO HangSanXuat (MaHSX, TenHSX) VALUES (?, ?)";
    $stmt = $conn->prepare($sql);
    if ($stmt) {
        $stmt->bind_param("ss", $maHSX, $tenHSX);
        if ($stmt->execute()) {
            $stmt->close();
            header("Location: brand.php");
            exit;
        } else {
            $tb = "Lỗi khi thêm hãng sản xuất: " . $stmt->error;
        }
        $stmt->close();
    } else {
        ...
        <div class="col-md-8">
            <div class="card shadow-lg pb-4">
                ...
                <div class="card-body col-md-8 mx-auto">
                    ...
                    <?php endif; ?>
                    <form method="POST" action="brand_create.php">
                        <table class="table1">
                            <tr>
                                <td class="col-md-3"><strong>Mã hãng:</strong></td>
                                <td>
                                    <input type="text" value="<?php echo layMaHSX($conn); ?>" 
                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="text" name="TenHSX" class="form-control" required 
                                           value="<?php echo isset($_POST['TenHSX']) ? htmlspecialchars($_POST['TenHSX']) : ''; ?>" />
                                </td>
                            </tr>
                        </table>
